/**
 * Smoke Test: Viewer User (lg@zueggcom.it)
 * Tests one viewer user - has only tool:email, tool:document
 */
...
setupE2ETests('smoke-viewer');

const USER = 'lg@zueggcom.it';

describe(`Viewer User: ${USER}`, () => {
  const skipTest = !isUserConfigured(USER);

  const toolsToTest = [
    // Should be ALLOWED (has tool:email, tool:document)
    { name: 'get_recent_emails', expected: 'allowed' },
    { name: 'get_documents', expected: 'allowed' },

    // Should be DENIED (no tool:erp-orders, tool:power, tool:erp, etc.)
    { name: 'get_last_weeks_orders', expected: 'denied' },
